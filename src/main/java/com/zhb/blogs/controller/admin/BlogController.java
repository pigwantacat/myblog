...
public class BlogController {
    ...
    public String searchBlogs(Blog blog, @RequestParam(required = false,defaultValue = "1",value = "pagenum")int pagenum, Model model){
        ...
        for (Blog b:allBlog) {//为获取到的每个博客的type的name赋值
            b.getType().setName(typeService.getTypeById(b.getTypeId()).getName());
        }
        //展示所有符合条件的博客，如果采用分页方式，当切换页面时会返回所有博客的上/下一页
        PageHelper.startPage(pagenum, allBlog.size());
        //得到分页结果对象
        PageInfo pageInfo = new PageInfo(allBlog);
        model.addAttribute("pageInfo", pageInfo);
        model.addAttribute("msg", "查询成功");
        setTypeAndTag(model);
        return "admin/blogs :: blogList";//局部刷新admin/blogs路径下的blogList模块
    }
